...
void incoming_packet_add(int id,
   ...
   /* Fill in the data */
   p->id = id;
   p->payload = static_cast<uint8_t*>(malloc(MSG_SIZE - sizeof(uint16_t)));
...
void* buzz_stream_incoming_thread_tcp(void* args) {
   ...
   uint8_t* buf = static_cast<uint8_t*>(calloc(MSG_SIZE, 1));
...
int buzz_script_set(const char* bo_filename,
   ...
   VM = buzzvm_new(1000); // Fixed ID, should not be used by another robot.
